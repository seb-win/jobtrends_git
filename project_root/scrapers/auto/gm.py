...
PAGE_START = 1
JOB_LIST_KEY = 'div.card-job'

HEADERS = {
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    'accept-language': 'de-DE,de;q=0.9,en-US;q=0.8,en;q=0.7',
    'priority': 'u=0, i',
    'referer': 'https://search-careers.gm.com/en/jobs/?search=&team=Communications&pagesize=20',
...
DAILY_JOB_URL = 'https://search-careers.gm.com/en/jobs/'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        # Extract Job ID
        job_element = job.find('div', class_='card-job-actions js-job')
        job_id = job_element.get('data-id') if job_element else None

        # Extract Job Title
        job_title = job_element.get('data-jobtitle') if job_element else None

        # Extract Job Department
        department = job.select_one('.job-meta li:nth-of-type(1)')
        department = department.get_text(strip=True) if department else None

        # Extract Job Location
        location = job.select_one('.job-meta li:nth-of-type(2)')
        location = location.get_text(strip=True) if location else None

        # Extract Job URL
        job_url_element = job.select_one('.card-title a')
        job_url = job_url_element.get('href') if job_url_element else None  
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://search-careers.gm.com/' + job.get('link')
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
                ...
                if response:
                    soup = BeautifulSoup(response.text, 'html.parser')
                    job_text = soup.find('div', id='content').get_text()
