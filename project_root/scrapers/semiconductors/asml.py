...
MAX_JOBS_PER_PAGE = 100  # Maximum number of jobs per page
PAGE_START = 0  # Starting page for pagination
key_name = 'limit'  # Key name for limit parameter
JOBS_LIST_KEY = ['widgets', 0, 'content']  # Key path for extracting the list of job postings
TOTAL_JOBS_KEY = ['widgets', 0, 'total_item']  # Key path for extracting the total number of jobs

# Define job detail keys for customization, including optional fields
job_data_keys = {
    'created': ['job_date_posted'],
    'jobTitle': ['name'],
    'department': ['job_technical_fields'],
    'team': ['job_teams'],
    'location': ['job_city'],
    'country': ['job_country'],
    'contract': [],
    'id': ['id'],
    'link': ['url'],
    'career_level': ['job_experience_levels'],
...
json_data = {
    'context': {
        'page': {
            'uri': 'https://www.asml.com/en/careers/find-your-job',
        },
        'locale': {
            'country': 'us',
            'language': 'en',
        ...
        'user': {
            'uuid': '126200477-n8-k2-4n-1p-hgq5yaclviz5dd7an8ml-1732083231164',
        },
    },
    'widget': {
        'items': [
...
daily_job_url = 'https://discover-euc1.sitecorecloud.io/discover/v2/126200477'
...
def main():
    ...
    if USE_PAGINATION:
        page = PAGE_START
        while True:
            json_data['widget']['items'][0]['search']['offset'] = 100 * page  # Update json_data for pagination
