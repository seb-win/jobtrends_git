...
PAGE_START = 1
JOB_LIST_KEY = 'div.matchElement'

HEADERS = {
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    'accept-language': 'de-DE,de;q=0.9,en-US;q=0.8,en;q=0.7',
    'priority': 'u=0, i',
    'referer': 'https://www.renk.com/',
...
DAILY_JOB_URL = 'https://renk-group.softgarden.io/de/widgets/jobs'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        if "matchHeadline" in job.get("class", []):  # Skip the header row
            continue
    
        # Extract job ID safely
        job_id = job.get("id", "").replace("job_id_", "")

        # Extract date safely
        date_element = job.find("div", {"class": "matchValue date"})
        date = date_element.text.strip() if date_element else None

        # Extract job title and link safely
        title_element = job.find("div", {"class": "matchValue title"})
        title_tag = title_element.find("a") if title_element else None
        title = title_tag.text.strip() if title_tag else None
        relative_link = title_tag["href"] if title_tag and "href" in title_tag.attrs else None

        # Extract audience safely
        audience_element = job.find("div", {"class": "matchValue audience"})
        audience = audience_element.text.strip() if audience_element else None

        # Extract location safely
        location_element = job.find("span", {"class": "location-view-item"})
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://renk-group.softgarden.io/job/' + str(job_id)
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
