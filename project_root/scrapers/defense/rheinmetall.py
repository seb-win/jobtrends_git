...
KEY_NAME = 'limit'
JOBS_LIST_KEY = ['elements', 'areablock', 1, 'data', 'search', 'results']
TOTAL_JOBS_KEY = ['elements', 'areablock', 1, 'data', 'search', 'hits']

HEADERS = {
    'sec-ch-ua-platform': '"macOS"',
    'Referer': 'https://www.rheinmetall.com/en/career/jobs-application/vacancies?term=',
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://www.rheinmetall.com/' + job.get('link')
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
