...
PAGE_START = 1
JOB_LIST_KEY = 'div.opening'

HEADERS = {
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    'accept-language': 'de-DE,de;q=0.9,en-US;q=0.8,en;q=0.7',
    'priority': 'u=0, i',
    'referer': 'https://www.epirusinc.com/',
...
DAILY_JOB_URL = 'https://boards.greenhouse.io/embed/job_board'


params = {
    'for': 'epirus',
    'b': 'https://www.epirusinc.com/open-roles',
}


def process_jobs(job_postings, departments):
    """
    ...
    for job in job_postings:
        ...
        department = job.get('department_id')
        ...
        if job_details['department'] != '':
            job_details['department'] = departments[job_details['department']]
...
def fetch_job_list_page(url, headers, params, use_proxy=False):
    ...
    try:
        ...
        departments = {h3['id']: h3.text.strip() for h3 in soup.find_all('h3')}
        return job_postings, departments
...
def fetch_all_jobs():
    ...
    if USE_PAGINATION:
        ...
        while True:
            ...
            job_postings, departments = fetch_job_list_page(DAILY_JOB_URL, HEADERS, params, use_proxy=USE_PROXY_DAILY_LIST)
            if not job_postings:
                logging.info("No more job postings found, or failed to retrieve job postings.")
                break

            jobs = process_jobs(job_postings, departments)
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://boards.greenhouse.io/embed/job_app'
            params['token'] = str(job.get('id'))
            print(params)
            if job_link:
                response = fetch_url(
                    job_link,
