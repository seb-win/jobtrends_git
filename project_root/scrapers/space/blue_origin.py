...
# --------------------------------------
BUCKET_NAME = 'space_jobs'
FOLDER_NAME = 'blue_origin'
...
PAGE_START = 1
JOB_LIST_KEY = 'ul[aria-label="Jobs"] li'
...
DAILY_JOB_URL = 'https://www.blueorigin.com/careers/search'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        # Extract job details from <dt> and <dd> pairs
        details = job.find_all('div')
        for detail in details:
            ...
            if dt and dd:
                ...
                # Rename "Requisition ID" to "ID"
                if key == "Requisition ID":
                    key = "id"
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            cleaned_url = re.sub(r".*(?=/BlueOrigin)", "", job_link)
            job_link = 'https://blueorigin.wd5.myworkdayjobs.com/wday/cxs/blueorigin' + cleaned_url
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
