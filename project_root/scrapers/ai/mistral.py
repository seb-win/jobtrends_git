...
HEADERS = {
    ...
    'Referer': 'https://jobs.lever.co/mistral',
...
DAILY_JOB_URL = 'https://jobs.lever.co/mistral'
...
def process_jobs(job_posting_groups):
    """
    ...
    jobs = []
    for group in job_posting_groups:
        department = group.find('div', class_='posting-category-title')
        department = department.get_text(strip=True) if department else ""

        for job in group.find_all('div', class_='posting'):
            posting_name_tag = job.find("h5", attrs={"data-qa": "posting-name"})
            posting_name = posting_name_tag.get_text(strip=True) if posting_name_tag else ""
            posting_id = str(job.get("data-qa-posting-id", ""))
            link = job.find("a", class_="posting-title")["href"] if job.find("a", class_="posting-title") else ""
            categories = job.find("div", class_="posting-categories")
            workplace_type = ""
            commitment = ""
            location = ""
            
            if categories:
                spans = categories.find_all("span", class_="small-category-label")
                if len(spans) > 0:
                    workplace_type = spans[0].get_text(strip=True).replace("—", "").strip()
                if len(spans) > 1:
                    commitment = spans[1].get_text(strip=True)
                if len(spans) > 2:
                    location = spans[2].get_text(strip=True)

            job_details = {
                'jobTitle': posting_name,
                'id': posting_id,
