...
MAX_JOBS_PER_PAGE = 48
PAGE_START = 1
JOB_LIST_KEY = 'div.attrax-vacancy-tile'
...
DAILY_JOB_URL = 'https://careers.abbvie.com/en/jobs'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        link = job.find("a", class_="attrax-vacancy-tile__title")["href"]
        job_id = job["data-jobid"]
        location_div = job.find("div", class_="attrax-vacancy-tile__location-freetext")
        location = location_div.find("p", class_="attrax-vacancy-tile__item-value").text.strip()
        salary_div = job.find("div", class_="attrax-vacancy-tile__salary")
        salary = salary_div.find("p", class_="attrax-vacancy-tile__item-value").text.strip()
        job_function = job.find("div", class_="attrax-vacancy-tile__option-function-valueset").find("p").text.strip()
        experience_level = job.find("div", class_="attrax-vacancy-tile__option-experience-level-valueset").find("p").text.strip()
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link =  'https://careers.abbvie.com/' + job.get('link')
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
