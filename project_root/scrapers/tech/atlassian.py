...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://wac-cdn-a.atlassian.com/company/careers/all-jobs?team=&location=&search=',
...
DAILY_JOB_URL = 'https://wac-cdn-a.atlassian.com/endpoint/careers/listings'
...
def fetch_job_list_page(page):
    ...
    total_jobs = 0
    if page == PAGE_START:
        total_jobs = len(job_data)  # Total jobs = length of the response list

    # The entire JSON response is already the job list
    job_list = job_data if not JOBS_LIST_KEY else get_nested_value(job_data, JOBS_LIST_KEY)

    if not isinstance(job_list, list):
        logging.error("Unexpected response format: job data is not a list.")
