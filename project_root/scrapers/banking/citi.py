...
MAX_JOBS_PER_PAGE = 2000
PAGE_START = 1
JOB_LIST_KEY = '#search-results-list ul li'
...
params = {
    ...
    'FacetTerm': '',
    'FacetType': '0',
    'SearchResultsModuleName': 'SearchResults - Technology',
...
DAILY_JOB_URL = 'https://jobs.citi.com/search-jobs/results'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        title_element = job.find("h3", class_="search-results__job-title")
        ...
        id = link_element["data-job-id"] if link_element and "data-job-id" in link_element.attrs else "N/A"
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://jobs.citi.com/' + job.get('link')
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
                ...
                if response:
                    ...
                    job_text = soup.find('section', class_='job-description').get_text()
