...
DAILY_JOB_URL = 'https://www.palantir.com/api/lever/v1/postings'
...
JOB_DATA_KEYS = {
    ...
    'country': [],
    'contract': ['categories', 'commitment'],
    'id': ['id'],
    'link': ['urls', 'show'],
    'career_level': [],
    'employment_type': [],
    'description': ['content', 'description'],
    'salary': ['content', 'closing'],
    'responsibilities': ['content', 'lists']
}
...
def fetch_all_jobs():
    ...
    response = requests.get(DAILY_JOB_URL, params=PARAMS, headers=HEADERS)

    json_obj = json.loads(response.text)

    hasNext = json_obj['hasNext']
    jobs = json_obj['data']
    all_jobs = jobs


    while hasNext:
        offset = urllib.parse.unquote(json_obj['next'])
        params = {
            'state': 'published',
            'offset': offset,
        }
