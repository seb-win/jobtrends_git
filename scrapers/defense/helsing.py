...
PAGE_START = 1
JOB_LIST_KEY = 'a.group.relative.block'
...
DAILY_JOB_URL = 'https://helsing.ai/de/jobs'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        department = job.find('div', {'data-label': 'Type'}).get_text(strip=True)
        location = job.find('div', {'data-label': 'Location'}).get_text(strip=True)
        contract = job.find('div', {'data-label': 'Time'}).get_text(strip=True)
        link = job['href']
        title = job.find('div', {'data-label': 'Position'}).get_text(strip=True)
...
def fetch_job_list_page(url, headers, params, use_proxy=False):
    ...
    try:
        ...
        job_postings = soup.select('a.group.relative.block')
        return job_postings
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://helsing.ai' + job.get('link')
            ...
            if job_link:
                ...
                if response:
                    ...
                    # Get the script content (string or list of contents)
                        content = script.string or "".join(map(str, script.contents)) or "Empty script"
                        # Store the content in the list
                        script_contents.append(content)
                combined_script_content = "\n\n".join(script_contents)
                upload_job_details_to_gcs(combined_script_content, job_id, BUCKET_NAME, FOLDER_NAME)
