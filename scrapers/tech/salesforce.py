...
DAILY_JOB_URL = 'https://careers.salesforce.com/en/jobs/'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        ...
        job_title_element = job.select_one('.card-title a')
        ...
        job_id_element = job.select_one('.card-job-actions.js-job')
        job_id = job_id_element['data-id'] if job_id_element else None

        # Extract department (Sales)
        department_element = job.select_one('.card-subtitle')
        department = department_element.text.strip() if department_element else None

        # Extract job locations
        location_elements = job.select('.locations .list-inline-item')
        locations = [location.text.strip() for location in location_elements] if location_elements else None
...
def update_master_list_with_jobs(all_jobs, master_list):
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://careers.salesforce.com' + job.get('link')
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
                ...
                if response:
                    soup = BeautifulSoup(response.text, 'html.parser')
                    job_title = soup.find('section', class_='hero hero-job').get_text()
                    job_desc = soup.find('div', class_='container job-detail').get_text()
