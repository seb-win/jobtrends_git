...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://jobs.ashbyhq.com/mistral.ai',
...
JSON_PAYLOAD = {
    ...
    'variables': {
        'organizationHostedJobsPageName': 'mistral.ai',
    },
...
def _mistral_section_name(heading):
    heading_text = clean_text(heading)
    if not heading_text:
        return "other"

    normalized = heading_text.casefold().rstrip(":")
    if normalized in {"about mistral", "about us"}:
        return "about"
    if normalized in {"the role", "strategic public sector engagement", "relationship & ecosystem leadership", "relationship and ecosystem leadership", "growth, expansion & impact", "growth, expansion and impact", "user support and issue resolution", "equipment setup and technology operations", "account and license management", "training and continuous improvement"}:
        return "responsibilities"
    if normalized in {"what we offer", "benefits"}:
        return "benefits"
    if normalized == "bonus points":
...
def _html_to_sections(html_text):
    ...
    for element in soup.find_all(["h1", "h2", "h3", "h4", "p", "li"], recursive=True):
        ...
        if is_structural_heading or is_standalone_bold_heading:
            ...
            heading = text.rstrip(":")
            current = {
                "name": _mistral_section_name(heading),
                "heading": heading,
                "text_parts": [],
                "items": [],
            }
