...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://jobs.ashbyhq.com/runway-ml',
...
JSON_PAYLOAD = {
    ...
    'variables': {
        'organizationHostedJobsPageName': 'runway-ml',
    },
...
def replace_team_ids_with_team_names(master_list, team_list, job_ids):
    id_to_name = build_team_id_lookup(team_list)
    job_ids = set(job_ids)
    ...
    for job in master_list:
        ...
        if department in id_to_name:
            job['department'] = id_to_name[department]
...
def fetch_all_jobs():
    ...
    if USE_PAGINATION:
        ...
        job_data, total_jobs_from_response, all_teams = fetch_job_list_page(PAGE_START)
        if job_data:
            all_jobs.extend(job_data)
...
def update_master_list_with_jobs(jobs, master_list, team_list):
    """
    ...
    for job in jobs:
        ...
        if processed_jobs_count % 100 == 0:
            replace_team_ids_with_team_names(master_list, team_list, new_job_ids)
    ...
    return new_jobs_count, inactive_jobs_count, skipped_jobs_count, new_job_ids
...
def main():
    ...
    # Step 1: Fetch all jobs and teams
    ...
    new_jobs_count, inactive_jobs_count, skipped_jobs_count, new_job_ids = update_master_list_with_jobs(jobs, master_list, team_list)

    # Step 4: Replace team IDs with team names for newly added entries
    replace_team_ids_with_team_names(master_list, team_list, new_job_ids)

    # Step 5: Save the updated master list
    save_master_list(BUCKET_NAME, FOLDER_NAME, master_list)
