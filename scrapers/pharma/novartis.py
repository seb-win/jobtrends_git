...
PAGE_START = 0
JOB_LIST_KEY = 'tbody tr'

HEADERS = {
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    'accept-language': 'de-DE,de;q=0.9,en-US;q=0.8,en;q=0.7',
    'priority': 'u=0, i',
    'referer': 'https://www.novartis.com/careers/career-search?page=1',
...
DAILY_JOB_URL = 'https://www.novartis.com/careers/career-search'
...
def process_jobs(job_postings):
    ...
    for job in job_postings:
        try:
            title_td = job.find('td', class_='views-field-field-job-title')
            title = title_td.get_text(strip=True) if title_td else "N/A"
            
            unit_td = job.find('td', class_='views-field-field-job-business-unit')
            unit = unit_td.get_text(strip=True) if unit_td else "N/A"
            
            country_td = job.find('td', class_='views-field-field-job-country')
            country = country_td.get_text(strip=True) if country_td else "N/A"
            
            location_td = job.find('td', class_='views-field-field-job-work-location')
            location = location_td.get_text(strip=True) if location_td else "N/A"
            
            posted_date_td = job.find('td', class_='views-field-field-job-posted-date')
            posted_date = posted_date_td.get_text(strip=True) if posted_date_td else "N/A"

            title_link = title_td.find('a') if title_td else None
            job_url = 'https://www.novartis.com/' + title_link['href'] if title_link and 'href' in title_link.attrs else "N/A"

            job_id_match = re.search(r'req-(\d+)', job_url)
...
def fetch_all_jobs():
    ...
    if USE_PAGINATION:
        ...
        while True:
            ...
            if not job_postings:
                logging.info("Stopping pagination: No job postings found or unable to retrieve.")
                break
    ...
    for job in all_jobs:
        ...
        if existing_entry:
            ...
            if job_link:
                ...
                if response:
                    soup = BeautifulSoup(response.text, 'html.parser')
                    job_text = soup.find('div', class_='job_details_content').get_text()
