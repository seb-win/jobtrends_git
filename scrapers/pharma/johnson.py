...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://jj.wd5.myworkdayjobs.com/en-US/JJ',
...
DAILY_JOB_URL = 'https://jj.wd5.myworkdayjobs.com/wday/cxs/jj/JJ/jobs'
...
def fetch_job_list_page(page):
    ...
    if PAGINATION_MODE == 'page':
        ...
        json_load['firstItem'] = first_item
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            try:
                job_path = job.get('link')
                if not isinstance(job_path, str) or not job_path:
                    raise TypeError(f"Invalid link value: {job_path!r}")

                job_link = 'https://jj.wd5.myworkdayjobs.com/wday/cxs/jj/JJ' + job_path
            except TypeError as exc:
                skipped_jobs_count += 1
                logging.warning(f"Skipping job {job_id} due to invalid link: {exc}")
                continue
            ...
            # Fetch job details if a valid link is provided
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
                ...
                if response:
                    ...
                    job_root = json_text.get("jobPostingInfo", {})
                    org = json_text.get("hiringOrganization", {})
                    ...
                    cleaned_job = {
                        ...
                        "id": job_root.get("id", ""),
                        "job_description": clean_html_to_text(job_root.get("jobDescription", "")),
                        "location": job_root.get("location", ""),
                        "posted_on": job_root.get("postedOn", ""),
                        "remote_type": job_root.get("remoteType", ""),
                        "time_type": job_root.get("timeType", ""),
...
def main():
    ...
    logging.info(f"Total jobs skipped due to missing IDs or invalid links: {skipped_jobs_count}")
