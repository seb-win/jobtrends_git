...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://careers.roche.com/global/en/search-results?from=10&s=1',
    ...
    'x-csrf-token': '634a33553ec04352af55e4d1bd32fa67',
}

DAILY_JOB_URL = 'https://careers.roche.com/widgets'
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            cleaned_url = 'https://roche.wd3.myworkdayjobs.com/wday/cxs/roche/roche-ext' + re.sub(r".*(/job.*)/apply", r"\1", job_link)
            if job_link:
                response = fetch_url(
                    cleaned_url,
                    headers=HEADERS,
                ...
                if response:
                    ...
                    job_text = json_text['jobPostingInfo']['jobDescription']
