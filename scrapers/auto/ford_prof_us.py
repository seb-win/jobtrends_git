...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://jobs.ford.com/careers?query=&location=&sort_by=relevance',
...
DAILY_JOB_URL = 'https://jobs.ford.com/api/apply/v2/jobs'
...
def process_jobs(job_data, job_data_keys):
    ...
    for listing in job_data:
        ...
        timestamp = datetime.fromtimestamp(job_details['created'])
        formatted_date = timestamp.strftime('%Y%m%d')
        job_details['created'] = formatted_date
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            job_link = 'https://jobs.ford.com/api/apply/v2/jobs/' + str(job_id)
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
                ...
                if response:
                    ...
                    job_text = json_text['job_description']
