...
JOB_DATA_KEYS = {
    ...
    'link': ['MatchedObjectDescriptor', 'PositionID'],
...
def updated_url(url, offset):
    # Decode the HTML entities
    decoded_string = html.unescape(url)

    # Extract the JSON part from the URL
    base_url, json_data = decoded_string.split("data=", 1)
    ...
    try:
        ...
        print("Invalid JSON String:", json_data)
        raise

    # Modify the "FirstItem" value dynamically
    json_dict["SearchParameters"]["FirstItem"] = offset  # Example new value

    # Convert back to a JSON string
    updated_json_data = json.dumps(json_dict)

    # Reconstruct the URL
    updated_url = f"{base_url}data={urllib.parse.quote(updated_json_data)}"
...
def fetch_job_list_page(page):
    ...
    new_url = updated_url(DAILY_JOB_URL, first_item)
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            job_link = f'https://api-deutschebank.beesite.de/jobhtml/{job_id}.json' 
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
