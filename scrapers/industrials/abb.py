...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://careers.abb/global/en/search-results',
...
DAILY_JOB_URL = 'https://careers.abb/widgets'
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            if seqNo:
                ...
                if response:
                    jobs_json = json.loads(response.text)
                    simJobsInit = jobs_json['peopleAlsoViewed']['data']['results']
                    available = False
                    for sim in simJobsInit:
                        ...
                        json_data2 = {
                            ...
                            'pageName': 'job',
                            'ddoKey': 'peopleAlsoViewed',
                            'jobSeqNo': sim['jobSeqNo'],
                        }   

                        response = requests.post('https://careers.abb/widgets', headers=HEADERS, json=json_data2)
                        jobs_json = json.loads(response.text)
                        simJobsSec = jobs_json['peopleAlsoViewed']['data']['results']
                        for job in simJobsSec:
                            if seqNo == job['jobSeqNo']:
                                job_text = str(job)
