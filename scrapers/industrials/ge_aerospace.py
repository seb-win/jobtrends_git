...
HEADERS = {
    ...
    'priority': 'u=1, i',
    'referer': 'https://careers.geaerospace.com/global/en/search-results',
    ...
    'x-csrf-token': '5ed87389bc4f44a98e3b7b1567ce44fc',
}

DAILY_JOB_URL = 'https://careers.geaerospace.com/widgets'
...
def update_master_list_with_jobs(jobs, master_list):
    ...
    for job in jobs:
        ...
        if existing_entry:
            ...
            pattern = r".*(/GE_ExternalSite.*?)(?:/apply|$)"
            match = re.search(pattern, job_link)
            print(f'Match: {match.group(1)}')
            job_link = 'https://geaerospace.wd5.myworkdayjobs.com/wday/cxs/geaerospace' + match.group(1) if match else None
            if job_link:
                response = fetch_url(
                    job_link,
                    headers=HEADERS,
